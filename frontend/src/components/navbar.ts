...
//             Play
//           </a>
// 		  <a href="#/statistics" data-route="/statistics" data-nav
//              class="text-sm font-medium text-gray-700 hover:text-gray-900
//                     px-3 py-1.5 rounded-xl hover:bg-gray-100 active:bg-gray-200
//                     focus:outline-none focus-visible:ring-2 focus-visible:ring-blue-500/60 transition">
//             Statistics
...
// 	const setActive = () => {
// 		const current = location.hash.replace(/^#/, '') || '/';
// 		// ⭐ Ne cible QUE les liens du menu (Home/Play), pas le logo
// 		nav.querySelectorAll<HTMLAnchorElement>('a[data-route][data-nav]').forEach(a => {
// 		  const isActive = a.dataset.route === current || (current === '/' && a.dataset.route === '/accueil');
// 		  a.classList.toggle('bg-gray-100', isActive);
// 		});
...
// 		incrementNotification: (d?: number) => void;
...
// 		const user = getSavedUser<{ username: string; avatarPath?: string; userId: string }>();
// 		if (!user) {
// 			right.innerHTML = `
//         <a href="#/connexion" data-route="/connection"
//            class="text-sm font-medium text-gray-700 hover:text-gray-900
...
// 		const btn = right.querySelector<HTMLButtonElement>('#btn-profile')!;
// 		const { attachProfileMenu } = await import('./profileMenu');
// 		menuCtrl = attachProfileMenu(btn, user, {
// 			notificationCount: 0,
// 			onLogoutSuccess: () => { renderRight(); setActive(); }
// 		});
// 		btn.addEventListener('click', (e) => { e.preventDefault(); menuCtrl?.open(); });
// 	};
