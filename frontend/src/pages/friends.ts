...
async function renderFriendsList(saved: AppUser) 
{
    ...
    for (let i = 0; i < data.length; i++) 
    {
      const p = data[i];
      html += `
        <li class="flex items-center gap-3 pr-2 py-2" data-id="${p.friendId}">
          <img src="${p.friendAvatarPath || '/avatar/default.png'}"
          ...
          <span class="inline-flex items-center gap-1 text-xs ${p.isOnline ? 'text-green-700 bg-green-100' : 'text-gray-700 bg-gray-100'} px-2 py-0.5 rounded-full">
            <span class="h-2 w-2 rounded-full ${p.isOnline ? 'bg-green-500' : 'bg-gray-400'}"></span>
    ...
    listEl.addEventListener('click', async (ev) => 
    {
      ...
      try {
        ...
        setStatusMessage(msg, `You have removed ${friend.friendUsername} from your friends`, 'success');
    ...
    listEl.querySelector<HTMLButtonElement>('button.profile')!.addEventListener('click', async (e) => {
        const btn = e.currentTarget as HTMLButtonElement;
        lockButton(btn, true);
        clearStatusMessage(msg);
        try {
